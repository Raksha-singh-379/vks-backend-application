...
import { EntityArrayResponseType, SocietyAssetsService } from '../service/society-assets.service';
...
@Component({
  selector: 'jhi-society-assets',
  templateUrl: './society-assets.component.html',
})
export class SocietyAssetsComponent implements OnInit {
  ...
  trackId = (_index: number, item: ISocietyAssets): number => this.societyAssetsService.getSocietyAssetsIdentifier(item);

  ngOnInit(): void {
    this.load();
  }

  delete(societyAssets: ISocietyAssets): void {
    const modalRef = this.modalService.open(SocietyAssetsDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.societyAssets = societyAssets;
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.societyAssets = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: ISocietyAssets[] | null): ISocietyAssets[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    return this.societyAssetsService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
