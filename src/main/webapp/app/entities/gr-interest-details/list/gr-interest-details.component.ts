...
import { EntityArrayResponseType, GRInterestDetailsService } from '../service/gr-interest-details.service';
...
@Component({
  selector: 'jhi-gr-interest-details',
  templateUrl: './gr-interest-details.component.html',
})
export class GRInterestDetailsComponent implements OnInit {
  ...
  trackId = (_index: number, item: IGRInterestDetails): number => this.gRInterestDetailsService.getGRInterestDetailsIdentifier(item);

  ngOnInit(): void {
    this.load();
  }

  delete(gRInterestDetails: IGRInterestDetails): void {
    const modalRef = this.modalService.open(GRInterestDetailsDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.gRInterestDetails = gRInterestDetails;
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.gRInterestDetails = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IGRInterestDetails[] | null): IGRInterestDetails[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    return this.gRInterestDetailsService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
