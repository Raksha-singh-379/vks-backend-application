...
import { EntityArrayResponseType, ExpenditureTypeService } from '../service/expenditure-type.service';
...
export class ExpenditureTypeComponent implements OnInit {
  ...
  trackId = (_index: number, item: IExpenditureType): number => this.expenditureTypeService.getExpenditureTypeIdentifier(item);

  ngOnInit(): void {
    this.load();
  }

  delete(expenditureType: IExpenditureType): void {
    const modalRef = this.modalService.open(ExpenditureTypeDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.expenditureType = expenditureType;
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.expenditureTypes = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IExpenditureType[] | null): IExpenditureType[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    return this.expenditureTypeService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
