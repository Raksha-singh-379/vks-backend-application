...
import { EntityArrayResponseType, AccountMappingService } from '../service/account-mapping.service';
...
export class AccountMappingComponent implements OnInit {
  ...
  trackId = (_index: number, item: IAccountMapping): number => this.accountMappingService.getAccountMappingIdentifier(item);

  ngOnInit(): void {
    this.load();
  }

  delete(accountMapping: IAccountMapping): void {
    const modalRef = this.modalService.open(AccountMappingDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.accountMapping = accountMapping;
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.accountMappings = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IAccountMapping[] | null): IAccountMapping[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    return this.accountMappingService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
