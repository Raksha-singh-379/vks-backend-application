...
import { EntityArrayResponseType, LoanDetailsService } from '../service/loan-details.service';
...
export class LoanDetailsComponent implements OnInit {
  ...
  trackId = (_index: number, item: ILoanDetails): number => this.loanDetailsService.getLoanDetailsIdentifier(item);

  ngOnInit(): void {
    this.load();
  }

  delete(loanDetails: ILoanDetails): void {
    const modalRef = this.modalService.open(LoanDetailsDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.loanDetails = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: ILoanDetails[] | null): ILoanDetails[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    return this.loanDetailsService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
