...
import { EntityArrayResponseType, SocietyAssetsDataService } from '../service/society-assets-data.service';
...
@Component({
  selector: 'jhi-society-assets-data',
  templateUrl: './society-assets-data.component.html',
})
export class SocietyAssetsDataComponent implements OnInit {
  ...
  trackId = (_index: number, item: ISocietyAssetsData): number => this.societyAssetsDataService.getSocietyAssetsDataIdentifier(item);

  ngOnInit(): void {
    this.load();
  }

  delete(societyAssetsData: ISocietyAssetsData): void {
    const modalRef = this.modalService.open(SocietyAssetsDataDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.societyAssetsData = societyAssetsData;
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.societyAssetsData = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: ISocietyAssetsData[] | null): ISocietyAssetsData[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    return this.societyAssetsDataService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
