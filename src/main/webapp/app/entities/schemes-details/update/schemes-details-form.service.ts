 ...
 */
type FormValueOf<T extends ISchemesDetails | NewSchemesDetails> = Omit<T, 'lastModified' | 'createdOn'> & {
...
type SchemesDetailsFormDefaults = Pick<NewSchemesDetails, 'id' | 'reinvestInterest' | 'lastModified' | 'createdOn' | 'isDeleted'>;
...
type SchemesDetailsFormGroupContent = {
  ...
  society: FormControl<SchemesDetailsFormRawValue['society']>;
};
...
export class SchemesDetailsFormService {
  createSchemesDetailsFormGroup(schemesDetails: SchemesDetailsFormGroupInput = { id: null }): SchemesDetailsFormGroup {
    ...
    return new FormGroup<SchemesDetailsFormGroupContent>({
      ...
      society: new FormControl(schemesDetailsRawValue.society),
    });
  }
