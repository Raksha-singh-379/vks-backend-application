...
import { EntityArrayResponseType, LoanWatapDetailsService } from '../service/loan-watap-details.service';
...
@Component({
  selector: 'jhi-loan-watap-details',
  templateUrl: './loan-watap-details.component.html',
})
export class LoanWatapDetailsComponent implements OnInit {
  ...
  trackId = (_index: number, item: ILoanWatapDetails): number => this.loanWatapDetailsService.getLoanWatapDetailsIdentifier(item);

  ngOnInit(): void {
    this.load();
  }

  delete(loanWatapDetails: ILoanWatapDetails): void {
    const modalRef = this.modalService.open(LoanWatapDetailsDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.loanWatapDetails = loanWatapDetails;
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.loanWatapDetails = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: ILoanWatapDetails[] | null): ILoanWatapDetails[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    return this.loanWatapDetailsService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
