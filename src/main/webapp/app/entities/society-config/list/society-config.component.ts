...
import { EntityArrayResponseType, SocietyConfigService } from '../service/society-config.service';
...
export class SocietyConfigComponent implements OnInit {
  ...
  trackId = (_index: number, item: ISocietyConfig): number => this.societyConfigService.getSocietyConfigIdentifier(item);

  ngOnInit(): void {
    this.load();
  }

  delete(societyConfig: ISocietyConfig): void {
    const modalRef = this.modalService.open(SocietyConfigDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.societyConfig = societyConfig;
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.societyConfigs = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: ISocietyConfig[] | null): ISocietyConfig[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    return this.societyConfigService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
